// A2 probe only: host approval and per-call Operations, not a product permission system.
...
export interface ProbeApproval {
  readonly operationId: string;
  readonly parametersDigest: string;
...
export interface ToolProbeOptions {
  binding: ToolBinding;
  deadline: () => number;
  authorize: (operation: ToolOperation, signal: AbortSignal) => Promise<ProbeApproval | undefined>;
...
export function createToolProbe(options: ToolProbeOptions) {
  ...
  function guard<S extends ToolDefinition['parameters'], D, State>(
    ...
    return {
      ...
      async execute(toolCallId, params, inputSignal, onUpdate, ctx) {
        ...
        let approval: ProbeApproval | undefined;
        let open = true;
