...
process.on('message', raw => {
  ...
  if (m.body.type === 'init') {
    ...
    else if (mode === 'native-outside') send({ type: 'session-reference', nativeRef: config.workspace + '/escape.jsonl' }, 'reference');
    else if (mode === 'native-unreserved') send({ type: 'ready', resourceLock: config.resources.id, nativeRef: config.sessions + '/unreserved.jsonl' }, 'ready');
