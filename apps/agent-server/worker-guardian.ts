...
function close(): Promise<void> {
  ...
  closing = (async () => {
    signalGroup('SIGTERM');
    for (let i = 0; i < 20 && (!exited || !groupGone()); i++) await delay(25);
    signalGroup('SIGKILL');
    for (let i = 0; i < 160 && (!exited || !groupGone()); i++) await delay(25);
...
if (!process.connected) await close();
  ...
  const sender = new IpcSender((message, callback) => worker!.send(message, callback));
  worker.on('error', () => { exited = true; void close(); });
  worker.on('close', () => { exited = true; sender.close(); void close(); });
  ...
  process.on('message', raw => {
    try { const message = parseEnvelope(raw);
      if (message.instanceId !== spec.instanceId || message.runtimeBindingId !== spec.runtimeBindingId || closing) throw new Error('stale_host');
      void sender.send(message).catch(() => close());
    } catch { void close(); }
  });
}
