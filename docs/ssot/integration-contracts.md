...
[最小桌面契约](c-desktop-contract.md) 已通过显式合成驱动连接 Electron sandbox Renderer、受限 preload、独立 Node 宿主及原 Worker。新增的有限正文快照只是展示索引；工具/审批/取消/成果仍取产品权威状态。C 不实现流式 delta、原生历史重建或完整第 2.1 节资源命令；证据和平台限制见 [实际验证](../validation/c-desktop-2026-09-23.md)。
