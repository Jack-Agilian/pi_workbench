...
代码提交 `61ad838ea601ef22f114a44ea533d3aff91517f0` 将 R02 的跨 Run 固定绑定、旧观察拒绝、迟到操作原归属和快照交接接入真实产品 SQLite；R07 的最低写准入使用全局单写及数据库约束，见 [17+5 项验证](../validation/b-2026-09-22.md)。R04 的实际 Pi write 经过持久批准再执行并登记真实成果。没有另写 Pi 消息/Session 树。

R03 成功替换已连接产品绑定，三类失败仍只有 A1 单独探针，完整产品恢复待后续 B 验证。真实 Worker/IPC、资源锁/权限完整绑定与清理监督没有实现；合成故障不算平台崩溃实测。R08 的三个 M0 Gate 仍 pending。
